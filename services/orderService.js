...
const orderService = {
  ...
  getOrdersByStatus: async (status, hours = 6) => {
    try {
      const headers = await getAuthHeaders();
      const response = await fetch(`${API_BASE_URL}/status/${status}?hours=${hours}`, {
        method: 'GET',
        headers,
      });
    ...
    getCreatedOrders: async (hours = 5) => {
        try {
          const headers = await getAuthHeaders();
          console.log(`📡 Envoi requête API (CREATED) : ${API_BASE_URL}/status/CREATED?hours=${hours}`);
    
          const response = await fetch(`${API_BASE_URL}/status/CREATED?hours=${hours}`, {
            method: 'GET',
            headers,
          });
  ...
  assignOrderToCourier: async (orderId, courierId) => {
    try {
      ...
      if (!response.ok) throw new Error(responseData.message || 'Impossible de prendre la commande');
 ...
 getAssignedOrders: async (courierId) => {
    ...
    updateOrderStatus: async (orderId, status) => {
        try {
          const headers = await getAuthHeaders();
          console.log(`📡 Envoi requête API (UPDATE STATUS) : ${API_BASE_URL}/${orderId}/status`);
