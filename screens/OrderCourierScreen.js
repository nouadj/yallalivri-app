      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.orderCard}>
            ...
            <Text style={styles.orderText}>💰 {item.totalAmount} {t('amount.dzd')}</Text>
            <Text style={styles.orderStatus}>{t('orders.status')}: {item.status}</Text>
            <Text style={styles.orderDate}>🕒 {t('orders.updated_at')}: {new Date(item.updatedAt).toLocaleString()}</Text>
            <Text style={styles.orderDate}>📅 {t('orders.created_at')}: {new Date(item.createdAt).toLocaleString()}</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
            ...
            <TouchableOpacity style={styles.takeButton} onPress={() => takeOrder(item.id)}>
              <Text style={styles.buttonText}>✅ {t('buttons.take_order')}</Text>
            </TouchableOpacity>
          </View>
        )}
      />
